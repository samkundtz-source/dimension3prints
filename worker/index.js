...
async function handleOvertureBuildings(request, env) {
  if (request.method !== 'POST') return jsonResponse({ error: 'Method not allowed' }, 405);

  const ip = getClientIP(request);
  const rl = await checkPublicRateLimit(env, ip, 'overture-buildings', 15, 60);
  if (rl.blocked) return jsonResponse({ error: 'Too many requests.' }, 429, { 'Retry-After': String(rl.retryAfter) });
  ...
  if (!env.OVERTURE_BUILDINGS) return jsonResponse({ features: [], note: 'OVERTURE_BUILDINGS R2 bucket not configured' });
  ...
  await Promise.all(tileKeys.map(async key => {
    try {
      const obj = await env.OVERTURE_BUILDINGS.get(key);
      if (!obj) return;
